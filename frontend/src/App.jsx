...
function App() {
  ...
  return (
    <div style={{ padding: '40px', fontFamily: 'sans-serif' }}>
      ...
      <button onClick={() => { setToken(''); localStorage.removeItem('token'); }}>Cerrar sesión</button>
      
      <hr style={{ margin: '20px 0' }} />
      
      <h3>Buscador (Prueba de Inyección SQL)</h3>
      ...
      <p style={{ fontSize: '14px', color: '#666' }}>
        <em>Intenta inyectar algo como <code>' OR 1=1 --</code>. Como usamos consultas parametrizadas, solo buscará literalmente ese texto y no atacará la base de datos.</em>
      </p>
      ...
      <ul style={{ listStyle: 'none', padding: 0 }}>
        {products.length === 0 ? <p>No se encontraron productos.</p> : null}
        {products.map(p => (
          <li key={p.id} style={{ margin: '10px 0', padding: '10px', border: '1px solid #ccc' }}>
            <strong>{p.name}</strong> - ${p.price} 
            <button onClick={() => handleDelete(p.id)} style={{ marginLeft: '15px', color: 'red', cursor: 'pointer' }}>
              Eliminar
            </button>
          </li>
        ))}
